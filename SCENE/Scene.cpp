...
void Scene::prepare(Box& box, std::vector<Shader> shaders) {
    ...
    lightUBO = UniformMemory::UBO(0, {
            UniformMemory::newStruct({ // dir light
            ...
            UniformMemory::Type::SCALAR, // no point lights
            ...
            UniformMemory::Type::SCALAR, // no spot lights
...
void Scene::renderShader(Shader shader, bool applyLighting) {
    ...
    if (applyLighting) {
        ...
        noActiveLights = 0;
        for (unsigned int i = 0; i < noLights; i++) {
            if (States::indexActive(&activeSpotLights, i)) {
